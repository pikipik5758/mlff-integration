...
class VideoProcessor:
    def __init__(self, source, lane_kiri, lane_kanan):
        self.source = source  # bisa int (kamera) atau str (path video)
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print(f"ERROR: Tidak bisa membuka video: {self.source}")
            return
...
def main():
    ...
    parser.add_argument("--camera", type=int, default=None, help="Index kamera (misal: 0, 1, 2)")
    args = parser.parse_args()

    if args.camera is None and args.video is None:
        print("ERROR: Gunakan --video atau --camera")
        return
    ...
    processor = VideoProcessor(source, lane_kiri, lane_kanan)
    processor.run()
